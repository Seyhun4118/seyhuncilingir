...
{
	if($dem==1){
		Header( "Location:../../index.php?demo=ok" );
		exit;
	}
	...
	$tnot = htmlspecialchars(trim($_POST[ 'not' ]));
	...
	if ( $update )
	{
		...
		Header( "Location:../../phpmail/randevu.php?status=ok" );
...
if ( isset( $_POST[ 'yorumkonu' ] ) )
{
	...
	if ($yorum_cins==1) {
		...
		$link = $settingsprint['ayar_siteurl'].seo('urunler-'.$urunwrite['urun_baslik']).'-'.$urunwrite['urun_id'];
...
if ( isset( $_POST[ 'bolgekategoriduzenle' ] ) )
{
	...
	if ( $_FILES[ 'resim' ][ "size" ] > 0 )
	{
		$uploads_dir = '../assets/img/bolgeler';
		@$tmp_name = $_FILES[ 'resim' ][ "tmp_name" ];
		@$name = $_FILES[ 'resim' ][ "name" ];
		$benzersizsayi1 = rand( 20000, 32000 );
		$benzersizsayi2 = rand( 20000, 32000 );
	...
	if ( $update )
	{
		...
		Header( "Location:../bolge-kategoriler.php?status=ok" );
	}
	else
	{

		Header( "Location:../bolge-kategoriler.php?status=no" );
	}
}
if ( isset( $_POST[ 'genelayar' ] ) )
{
	...
	if ( $_FILES[ 'ayar_flogo' ][ "size" ] > 0 )
	{
		$uploads_dir = '../assets/img/genel';
		@$tmp_name = $_FILES[ 'ayar_flogo' ][ "tmp_name" ];
		$benzersizsayi4 = rand( 20000, 32000 );
		$uzanti = '.jpg';
		$refimgyol      = substr( $uploads_dir, 3 ) . "/" . $benzersizsayi4 . $uzanti;
		...
		if ( $update )
		{
			$resimsilunlink = $_POST[ 'eskiyol_flogo' ];
			unlink( "../$resimsilunlink" );
	...
	if ( $_FILES[ 'ayar_fav' ][ "size" ] > 0 )
	{
		...
		$ayarkaydet = $db->prepare(
			...
			ayar_firmahakkinda=:firmahakkinda,
			ayar_mobil=:mobil,
			ayar_title3=:ayar_title3,
			ayar_kod=:kod,
			ayar_harita=:harita
			WHERE ayar_id=0"
		);
		$update     = $ayarkaydet->execute(
			array(
				...
				'firmahakkinda'    => $_POST[ 'ayar_firmahakkinda' ],
				'mobil'    => "#".$_POST[ 'ayar_mobil' ],
				'ayar_title3'    => "#".$_POST[ 'ayar_title3' ],
				'kod'     => $_POST[ 'ayar_kod' ],
...
if ( isset( $_POST[ 'slaytduzenle' ] ) )
{
	...
	if ( $_FILES[ 'mobil' ][ "size" ] > 0 )
	{

		$uploads_dir = '../assets/img/slayt';
		@$tmp_name = $_FILES[ 'mobil' ][ "tmp_name" ];
		$benzersizsayi4 = rand( 20000, 32000 );
		$uzanti = '.jpg';
		$refimgyol3     = substr( $uploads_dir, 3 ) . "/" . $benzersizsayi4 . $uzanti;
...
if ( isset( $_POST[ 'hizmetduzenle' ] ) )
{
	...
	{
		$uploads_dir = '../assets/img/hizmetler';
		@$tmp_name = $_FILES[ 'hizmet_icon' ][ "tmp_name" ];
		@$name = $_FILES[ 'hizmet_icon' ][ "name" ];
		$benzersizsayi1 = rand( 20000, 32000 );
		$benzersizsayi2 = rand( 20000, 32000 );
		$uzanti='.jpg';
		$benzersizad    = $benzersizsayi1 . $benzersizsayi2;
		$refimgyoli      = substr( $uploads_dir, 3 ) . "/" . $benzersizad . $uzanti;
		@move_uploaded_file( $tmp_name, "$uploads_dir/$benzersizad$uzanti" );
...
if ( isset( $_POST[ 'slaytekle' ] ) )
{
	...
	$uploads_dir = '../assets/img/slayt';
	$tmp_name = $_FILES[ 'mobil' ][ "tmp_name" ];
	$benzersizsayi10 = rand( 20000, 32000 );
	$benzersizsayi20 = rand( 20000, 32000 );
	$uzanti = '.jpg';
	$benzersizad0    = $benzersizsayi10 . $benzersizsayi20 ;
	$refimgyol12      = substr( $uploads_dir, 3 ) . "/" . $benzersizad0 . $uzanti;
	move_uploaded_file( $tmp_name, "$uploads_dir/$benzersizad0$uzanti" );
...
if ( $_GET[ 'randevusil' ] == "ok" )
{
	...
	$sil     = $db->prepare( "DELETE from teklif where id=:teklif_id" );
	$kontrol = $sil->execute(
		array(
			'teklif_id' => $_GET[ 'randevu_id' ]
		)
	);
...
if ( $_GET[ 'bolgeresimsil' ] == "ok" )
{
	...
	if ( $kontrol )
	{
		...
		Header( "Location:../bolge-resim-duzenle.php?bolge_id=$urun&status=ok" );
	}
	else
	{

		Header( "Location:../bolge-resim-duzenle.php?bolge_id=$urun&status=no" );
	}
}
...
if ( isset( $_POST[ 'omenuduzenle' ] ) )
{
	...
	$omenueditx=$db->prepare("SELECT * from omenu where omenu_id=:hizmet_id");
	$omenueditx->execute(array(
		'hizmet_id' => $_POST['omenu_id']
	));
	$omenux=$omenueditx->fetch(PDO::FETCH_ASSOC);

	$omenuedit=$db->prepare("SELECT * from omenu where omenu_ust=:hizmet_id");
	$omenuedit->execute(array(
		'hizmet_id' => $omenux['omenu_id']
	));
	$omenu=$omenuedit->fetch(PDO::FETCH_ASSOC);
		
	$omenuSay=$omenuedit->rowCount();


	
		if($omenuSay>=1){
		$Durum=$omenu[ 'omenu_id' ];
		} else {
			if($_POST[ 'omenu_ust' ]!=$omenu[ 'omenu_ust' ]) {
				$Durum=0;
			} else {
				$Durum=$_POST[ 'omenu_id' ];
			}
		}
